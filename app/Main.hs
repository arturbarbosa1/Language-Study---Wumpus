...
-- FIXME: warn user about partner's connection loses.
module Main where
...
loopPlayer sock gameStore = forever $ do
  ...
  if wumpusWins
  then do
    unless gameClosed $ void $ send sock $ "You lose!\n"
    putMVar gameStore game
  else if playerWins
  then do
    unless gameClosed $ void $ send sock $ "You win!]n"
    putMVar gameStore game {gameClosed = True}
  else do
...
loopWumpus sock gameStore = forever $ do
  ...
  if wumpusWins
  then do
    unless gameClosed $ void $ send sock $ "You win!\n"
    putMVar gameStore game {gameClosed = True}
  else if playerWins
  then do
    unless gameClosed $ void $ send sock $ "You lose!]n"
    putMVar gameStore game
  else do
